...
function applyVersion(){document.querySelectorAll('.app-version,.app-version-inside').forEach(e=>e.textContent='v'+APP_VERSION)}
...
function openRecipeAdd(r){
  const ingredients=Array.isArray(r?.ingredients)?r.ingredients.filter(Boolean):[];
  $('#recipeAddTitle').textContent=r?.name||t('recipeAddTitle');
  $('#recipeAddHint').textContent=ingredients.length?t('recipeSelectIngredients'):t('recipeNoIngredients');
  const select=$('#recipeAddList');
  select.innerHTML='';
  (state.lists||[]).forEach(list=>{
    const option=document.createElement('option'); option.value=list.id; option.textContent=list.name==='Supermercado'?(state.lang==='PT'?'Supermercado':'Supermarket'):list.name;
    if(state.activeList?.id===list.id)option.selected=true; select.appendChild(option);
  });
  const box=$('#recipeIngredientChoices'); box.innerHTML='';
  ingredients.forEach((ingredient,index)=>{
    const label=document.createElement('label'); label.className='recipe-choice';
    const cb=document.createElement('input'); cb.type='checkbox'; cb.checked=true; cb.value=ingredient; cb.dataset.index=String(index);
    const span=document.createElement('span'); span.textContent=ingredient;
...
function closeRecipeAdd(){$('#recipeAddModal').hidden=true}
async function confirmRecipeAdd(){
  const r=state.recipes.find(x=>x.id===window.__recipeToAddId);
  if(!r)return;
  const listId=$('#recipeAddList').value;
  const list=state.lists.find(x=>x.id===listId);
  const selected=[...document.querySelectorAll('#recipeIngredientChoices input[type="checkbox"]:checked')].map(x=>x.value).filter(Boolean);
  $('#recipeAddError').textContent='';
  if(!list){$('#recipeAddError').textContent=t('connectionError');return}
  if(!selected.length){$('#recipeAddError').textContent=t('recipeAddNone');return}
  $('#recipeAddConfirm').disabled=true;
  try{
    for(const ingredient of selected){
      await api(`/api/lists/${list.id}/items`,{method:'POST',body:JSON.stringify({name:ingredient,category:'Outros',quantity:1,unit:'un.',price:null,store:list.name==='Casa'?'':''})});
    }
    state.activeList=list;
    closeRecipeAdd();
    await loadItems();
    toast(t('recipeAddedCount').replace('{count}',String(selected.length)));
  }catch(e){
    $('#recipeAddError').textContent=e.message||t('connectionError');
  }finally{$('#recipeAddConfirm').disabled=!selected.length}
}
function addRecipeToList(r){window.__recipeToAddId=r.id;openRecipeAdd(r)}

function showView(id){$$('.view').forEach(v=>v.hidden=v.id!==id);$$('.nav-item').forEach(n=>n.classList.toggle('active',n.dataset.view===id));window.scrollTo({top:0,behavior:'instant'});if(id==='historyView')loadHistory();if(id==='recipesView')loadRecipes()}
...
function bind(){
  ...
  $('#recipeAddClose').onclick=closeRecipeAdd;$('#recipeAddCancel').onclick=closeRecipeAdd;$('#recipeAddConfirm').onclick=confirmRecipeAdd;
